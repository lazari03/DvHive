...
<template>
  ...
  >
    <div
      class="md:w-3/5 text-white w-full h-fit relative flex flex-col justify-center items-center"
    >
      <h1
        v-if="$route.path !== '/faq'"
    ...
    <div class="relative md:w-4/5 flex justify-end w-full">
      <div class="justify-center flex w-full">
        <div class="relative md:w-4/5 flex justify-end items-end w-full">
          <div
            class="absolute w-4/6 h-5/6 bg-[#FDCE83] rounded-2xl translate-x-4 translate-y-3"
          ></div>
          <img :src="image" :alt="alt" class="relative w-[450px] h-64 rounded-2xl object-cover" />
        </div>
      </div>
    </div>
  </div>
